    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 rounded-lg pt-6 mb-6" style="background-color: black;">
            ...
            @forelse ($articles as $article)
                <x-card>
                    <a href="{{ route('admin.articles.show', $article) }}" class="font-bold text-xl text-black">{{ ($article->heading) }}</a>
                </x-card>
            @empty
            ...
            <div class="mt-6">
                <x-primary-button style="background-color: green;">
                    <a href="{{ route('admin.authors.edit', $author) }}" class="text-white">EDIT</a>
                </x-primary-button>
                <form action="{{ route('admin.authors.destroy', $author) }}" method="POST" style="display: inline;">
                    @method('delete')
                    @csrf
                    <x-primary-button  style="background-color: green;" onclick="return confirm('ARE YOU SURE YOU WANT TO DELETE?')">DELETE</x-primary-button>
                </form>
            </div>
        </div>
    </div>
